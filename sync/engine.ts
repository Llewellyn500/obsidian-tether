...
export class SyncEngine {
	...
	private async processLocalPath(path: string, vaultRootId: string) {
		...
		// Push is authoritative. A state entry is only valid when its Drive ID still exists
		// at the same path; otherwise recreate/update the expected remote item even when the
		// local mtime has not changed since the previous sync.
		...
		if (state && stat.mtime <= state.lastSyncedMtime) return;
	...
	private async collectRemoteMirrorItems(folderId: string, parentPath: string = '', depth: number = 0, items: RemoteMirrorItem[] = []): Promise<RemoteMirrorItem[]> {
		if (depth > 50) throw new Error('Maximum folder depth reached.');
		this.throwIfStopped();
	...
	private prepareRemoteMirrorDeletions(localPathSet: Set<string>, remoteItems: RemoteMirrorItem[]): RemoteMirrorItem[] {
		...
		if (localPathSet.size === 0) {
			...
			const msg = `Push paused remote deletions: this device returned no local files or folders, but Drive has ${remoteCount} item${remoteCount === 1 ? '' : 's'}. No remote files were deleted.`;
			console.error(msg);
			new Notice(msg);
			this.stats.failed++;
			this.stats.errors.push({ path: 'Remote deletions', message: msg });
		...
		if (remoteCount > 0 && staleItems.length / remoteCount >= CATASTROPHIC_DELETE_RATIO) {
			...
			new Notice(`Confirmed large push deletion batch: deleting ${staleItems.length} remote items.`);
		}

		// Deleting a stale folder also deletes its stale descendants. Keep only the
		// highest stale path in each branch so Drive receives one request per branch.
		const stalePaths = new Set(staleItems.map(item => item.path.toLowerCase()));
		return staleItems
			.filter(item => {
				const parts = item.path.toLowerCase().split('/');
				for (let i = 1; i < parts.length; i++) {
					if (stalePaths.has(parts.slice(0, i).join('/'))) return false;
				}
				return true;
			})
	...
	private async deleteRemoteMirrorItems(deletionRoots: RemoteMirrorItem[], remoteItems: RemoteMirrorItem[]) {
	...
	private removeDeletedRemoteState(rootPath: string, deletedDriveIds: Set<string>) {
		const normalizedRoot = rootPath.toLowerCase();
		for (const [path, entry] of Object.entries(this.stateManager.state)) {
			if (path === '__VAULT_ROOT__') continue;
